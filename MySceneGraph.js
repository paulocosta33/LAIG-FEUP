 ...
 */
MySceneGraph.prototype.displayScene = function(nodeName)
{
    if(nodeName != null)
    {
        var no = this.nodes[nodeName];

        for(var i = 0; i < no.children.length; i++)
        {
            this.scene.pushMatrix();
                this.scene.multMatrix(no.transformMatrix);
            //applymaterial(material);
            this.displayScene(no.children[i]);
            this.scene.popMatrix();
        }
        if(no.leaf != null)
        {
            no.leaf.object.display();
        }
    }
}
